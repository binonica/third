...
<form action="myInfoOK.jsp" method="post">
	...
	<!--
		radio와 checkbok는 같은 그룹끼리 반드시 같은 이름의 name 속성을 지정해야 그룹으로 묶인다.
		radio와 checkbok는 컨트롤 자체를 화면에 표시하는 기능밖에 없기 때문에 선택했을 때 다음 페이지로 넘겨줄 데이터를 value 속성을
		사용해서 넣어줘야 한다. => value 속성을 지정하지 않으면 선택한 경우 선택한 경우 무조건 "on"이 선택하지 않으면 null이 넘어간다.
	-->
